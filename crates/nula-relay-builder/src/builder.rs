//! Fluent builder for [`crate::MockRelay`].
...
use crate::policy::{AcceptAllReads, AcceptAllWrites, ReadPolicy, WritePolicy};
use crate::server::MockRelay;
...
fn default_storage() -> Arc<dyn NostrDatabase> {
    panic!(
        "MockRelayBuilder::run called without a storage backend \
         and the `memory` feature is off; supply one via \
         `MockRelayBuilder::storage(...)`"
    )
}
