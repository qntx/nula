...
//! with an in-process [`nula_storage_memory::MemoryDatabase`] (hot
...
use nula_storage::{
    Backend, DatabaseEventStatus, Events, Features, NostrDatabase, SaveEventStatus,
};
...
impl NostrDatabase for SqliteDatabase {
    ...
    fn save_event<'a>(
        &'a self,
        event: &'a Event,
    ) -> BoxFuture<'a, Result<SaveEventStatus, nula_storage::Error>> {
        Box::pin(async move {
    ...
    fn check_id<'a>(
        &'a self,
        event_id: &'a EventId,
    ) -> BoxFuture<'a, Result<DatabaseEventStatus, nula_storage::Error>> {
    ...
    fn event_by_id<'a>(
        &'a self,
        event_id: &'a EventId,
    ) -> BoxFuture<'a, Result<Option<Event>, nula_storage::Error>> {
        self.inner.memory.event_by_id(event_id)
    }

    fn count(&self, filter: Filter) -> BoxFuture<'_, Result<usize, nula_storage::Error>> {
        self.inner.memory.count(filter)
    }

    fn query(&self, filter: Filter) -> BoxFuture<'_, Result<Events, nula_storage::Error>> {
    ...
    fn negentropy_items(
        &self,
        filter: Filter,
    ) -> BoxFuture<'_, Result<Vec<(EventId, Timestamp)>, nula_storage::Error>> {
        self.inner.memory.negentropy_items(filter)
    }

    fn delete(&self, filter: Filter) -> BoxFuture<'_, Result<(), nula_storage::Error>> {
        Box::pin(async move {
    ...
    fn wipe(&self) -> BoxFuture<'_, Result<(), nula_storage::Error>> {
        Box::pin(async move {
...
async fn run_sql<F>(conn: &Arc<Mutex<Connection>>, op: F) -> Result<(), nula_storage::Error>
where
...
fn map_storage_err(e: Error) -> nula_storage::Error {
    nula_storage::Error::Backend(Box::new(e))
}
