...
use nula_relay::transport::{ConnectionMode, IntoWebSocketTransport, Message, WebSocketTransport};
...
async fn missing_subscriber_yields_protocol_violation() {
    ...
    assert!(matches!(
        err,
        nula_relay::transport::Error::ProtocolViolation { reason } if reason.contains("MockTransport")
    ));
}
...
async fn inbound_error_surfaces_on_stream() {
    ...
    handle
        .push_inbound_error(nula_relay::transport::Error::ConnectionClosed)
        .expect("push error");

    let err = task.await.expect("task joined");
    assert!(matches!(
        err,
        nula_relay::transport::Error::ConnectionClosed
    ));
}
