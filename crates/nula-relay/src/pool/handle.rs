...
use crate::pool::inner::{Inner, RelayEntry, spawn_forwarder};
...
impl RelayPool {
    ...
    /// [`crate::pool::RelayCapabilities::WRITE`].
    ///
    /// # Errors
    ///
    ...
    pub async fn send_event(&self, event: Event) -> Result<Output<EventId>, Error> {
        let urls = self
            .relays_with_any_capability(crate::pool::RelayCapabilities::WRITE)
            .await;
    ...
    /// [`crate::pool::RelayCapabilities::READ`].
    ///
    /// The pool spawns one driver task per (relay, subscription)
    /// pair which forwards events to the [`crate::pool::RelayPool`]'s
    ...
    pub async fn subscribe(
        ...
        let urls = self
            .relays_with_any_capability(crate::pool::RelayCapabilities::READ)
            .await;
        ...
        I: IntoIterator<Item = RelayUrl>,
    {
        ...
        let snapshot = self.relays().await;
        let mut handles: Vec<(RelayUrl, Result<SubscriptionHandle, crate::Error>)> =
    ...
    /// [`nula_core::boxed::BoxStream`] yields events from any relay,
    ...
    ) -> Result<nula_core::boxed::BoxStream<'static, (RelayUrl, Result<Event, crate::Error>)>, Error>
    {
        let urls = self
            .relays_with_any_capability(crate::pool::RelayCapabilities::READ)
            .await;
