//! Error surface for [`crate::RelayPool`].
//!
...
//! end up in [`crate::Output::failed`] rather than bubbling up here.

use nula_core::RelayUrl;
use thiserror::Error;

/// Errors raised by [`crate::RelayPool`] operations.
#[allow(
...
pub enum Error {
    ...
    /// [`crate::RelayPool::shutdown`] call or the last clone of the
    ...
    /// The pool already holds [`crate::RelayPoolOptions::max_relays`]
    ...
    /// could route it into [`crate::Output::failed`]. In practice
    ...
    /// [`crate::RelayPoolOptions::auto_save_events`]) bubbled up an
