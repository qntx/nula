...
//!
//! The crate is `publish = false`: it lives in the workspace strictly
//! as a dev-dependency for first- and third-party storage backends
//! and never reaches crates.io.
//!
//! # Quickstart
//!
//! ```rust,ignore
...
#![doc(html_root_url = "https://docs.rs/nula-storage-test-suite")]
#![forbid(unsafe_code)]
#![allow(
    clippy::expect_used,
    clippy::unwrap_used,
    ...
    reason = "test-only crate (publish = false); every panic surfaces as a test failure"
)]
